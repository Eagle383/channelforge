...
**Duplicate merging.** Automatic merges delete canonical channels when there is hard duplicate evidence: the same guide station ID, the same normalized name (`123 GO!` / `123GO!`), a plain channel plus its provider rename (`Duck Dynasty` / `Duck Dynasty by A&E`), or a strongly overlapping XMLTV programme lineup. Programme-lineup matching fingerprints same-title XMLTV entries in the same five-minute guide slot for every assigned source guide identity: `channel-id`, `tvg-id`, `tvc-guide-stationid`, source external ID, guide title/name, and channel name, including channels whose output uses Gracenote instead of the combined XMLTV guide. Short high-overlap schedules are enough to merge. Softer name matches and weaker guide clues stay on the duplicate-review page for a manual verdict. That review page buckets candidates as strong, medium, or weak so you can work guide/lineup evidence first and bulk-dismiss weak name-only noise. Each group is ordered by the same stream picker used for outputs, so source priority and provider order put the suggested keeper first.
