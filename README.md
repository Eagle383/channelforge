...
**Duplicate merging.** Automatic merges only delete canonical channels when there is hard duplicate evidence: the same guide station ID, the same normalized name (`123 GO!` / `123GO!`), or a plain channel plus its provider rename (`Duck Dynasty` / `Duck Dynasty by A&E`). Softer name matches and guide-data clues like matching `tvg-id` values or repeated long guide descriptions stay on the duplicate-review page for a manual verdict.
