...
**Stream selection.** At output time each channel picks a stream from its assigned source channels: healthy streams first, then a per-channel provider override if set, then an optional per-channel *preferred source*, then source priority order, then the provider order (Sources page, drag to reorder) as the tie-break inside combined feeds. If a stream starts failing health checks, the next refresh fails over automatically.
