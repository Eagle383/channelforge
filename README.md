...
- **Provider priority** — combined feeds carrying many providers (`samsung.*`, `stirr.*`, ...) pick the winning duplicate stream by a drag-and-drop provider order, with per-channel provider overrides when one channel needs a different source.
...
**Stream selection.** At output time each channel picks a stream from its assigned source channels: healthy streams first, then a per-channel provider override if set, then an optional per-channel *preferred source*, then source priority order, then the provider order (Sources page, drag to reorder) as the tie-break inside combined feeds. Provider order entries are kept even when a provider temporarily disappears, so the priority applies again when it returns. If a stream starts failing health checks, the next refresh fails over automatically.

**Duplicate merging.** Automatic merges delete canonical channels when there is hard duplicate evidence: the same guide station ID, the same normalized name (`123 GO!` / `123GO!`), a plain channel plus its provider rename (`Duck Dynasty` / `Duck Dynasty by A&E`), or a strongly overlapping programme lineup. Programme-lineup matching fingerprints same-title entries in the same five-minute guide slot from source XMLTV files and, when a Channels DVR server is configured, from Channels DVR's own guide JSON for Gracenote-backed rows. It keys those fingerprints by `channel-id`, `channel-number`, `tvg-id`, `tvg-chno`, `tvc-guide-stationid`, source external ID, guide title/name, and channel name. Short high-overlap schedules are enough to merge. Softer name matches and weaker guide clues stay on the duplicate-review page for a manual verdict. That review page buckets candidates as strong, medium, or weak so you can work guide/lineup evidence first and bulk-dismiss weak name-only noise. Each group is ordered by the same stream picker used for outputs, so source priority and provider order put the suggested keeper first.
